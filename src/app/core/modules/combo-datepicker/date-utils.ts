export default class DateUtils {
  static adjustTimezone(myDate: Date, myTimezone: any) {
    const offset = isNaN(myTimezone)
      ? new Date().getTimezoneOffset()
      : parseFloat(myTimezone) * 60;
    return new Date(myDate.getTime() + offset * 60 * 1000);
  }

  static parseIntStrict(num: string) {
    return num !== null && num !== '' && !isNaN(parseInt(num, 10))
      ? parseInt(num, 10)
      : null;
  }

  static getMaxDate(month: number, year: number) {
    let res = 31;
    if (month != null) {
  ...
  static parseDate(myDate: any, myTimezone: any) {
    let res = null;
