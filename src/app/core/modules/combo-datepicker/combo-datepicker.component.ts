...
export class ComboDatepickerComponent
  implements OnInit, OnChanges, ControlValueAccessor, Validator
{
  ...
  onChange() {
    ...
    if (
      this.selects.d.value &&
      !isNaN(this.selects.m.value) &&
      this.selects.y.value != null &&
      this.selects.y.value
    ) {
      const maxDay = DateUtils.getMaxDate(
        this.selects.m.value + 1,
        this.selects.y.value
      );
      ...
      if (this.selects.d.value > maxDay) {
        ...
        res = new Date(
          ...
          this.selects.d.value > maxDay ? maxDay : this.selects.d.value,
          hours,
          minutes,
          seconds,
          milliseconds
  ...
  writeValue(value: any): void {
    if (value) {
      ...
      if (typeof this.visible === 'boolean') {
        ...
        this.selects.y.value = this.visible[2]
          ? null
  ...
  updateYearList(month?: any) {
    // Parse parameter.
    month = DateUtils.parseIntStrict(month);

    this.selects.y.options = [];
    const isReverse =
      typeof this.yearOrder === 'string' && this.yearOrder.indexOf('des') === 0;
    for (
      let i = this._minDate.getFullYear();
      i <= this._maxDate.getFullYear();
      i++
    ) {
      const now = new Date();
      if (
        month + 1 < this._minDate.getMonth() &&
        now.getFullYear() === i &&
        month !== null &&
        ((isReverse && month > now.getMonth()) ||
          (!isReverse && month < now.getMonth()))
      ) {
        continue;
      }
  ...
  updateDateList(month: any, year: any) {
    // Parse parameters.
    month = DateUtils.parseIntStrict(month);
    year = DateUtils.parseIntStrict(year);
